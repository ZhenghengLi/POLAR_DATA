...
Long64_t HkIterator::find_entry_obox_(TTree* t_tree, Hk_Obox_T& t_branch, double gps_value) {
...
Long64_t HkIterator::find_entry_ibox_(TTree* t_tree, Hk_Ibox_T& t_branch, double gps_value) {
    Long64_t head_entry          = -1;
    bool     head_entry_found    = false;
    while (head_entry < t_tree->GetEntries()) {
        head_entry += 1;
...
bool HkIterator::open(const char* filename, const char* gps_begin, const char* gps_end) {
    if (t_file_in_ != NULL)
        return false;
...
void HkIterator::print_file_info() {
    if (t_file_in_ == NULL)
        return;
    char str_buffer[200];
...
double HkIterator::value_of_gps_str_(const string gps_str) {
    cmatch cm;
...
string HkIterator::get_filename() {
    return string(TSystem().BaseName(name_str_file_in_.c_str()));
}

double HkIterator::get_gps_value_first() {
    return gps_value_first_hk_ibox_;
}

double HkIterator::get_gps_value_last() {
...
Long64_t HkIterator::hk_obox_get_tot_entries() {
    return hk_obox_last_entry_ - hk_obox_first_entry_;
}

Long64_t HkIterator::hk_obox_get_cur_entry() {
...
Long64_t HkIterator::hk_ibox_get_tot_entries() {
    return hk_ibox_last_entry_ - hk_ibox_first_entry_;
}

Long64_t HkIterator::hk_ibox_get_cur_entry() {
