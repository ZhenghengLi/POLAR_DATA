...
int main(int argc, char** argv) {
	...
	while (phyEventFile.trigg_next()) {
		if (phyEventFile.trigg.pkt_count > 0 
				&& phyEventFile.trigg.lost_count == 0 
				&& phyEventFile.trigg.mode == 0x00FF)
			order_num++;
		if (order_num < number)
			continue;
		while (phyEventFile.event_next()) {
			cout << phyEventFile.event.trigg_index << " | " << phyEventFile.event.ct_num << " | ";
