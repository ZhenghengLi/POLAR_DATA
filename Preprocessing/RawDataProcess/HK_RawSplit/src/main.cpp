...
double read_gps_time(char* frame) {
    ...
    for (int i = 0; i < 6; i++) {
        frm_gps_time_ <<= 8;
        frm_gps_time_ += static_cast<uint8_t>(frame[250 + i]);
    }
...
int main(int argc, char** argv) {
    ...
    while (filelist.next_frame()) {
        ...
        for (int i = 0; i < 4; i++) {
            expected <<= 8;
            expected += static_cast<uint8_t>(filelist.data_buffer[256 + i]);
        }
        crc_32_.reset();
        crc_32_.process_bytes(filelist.data_buffer, 256);
