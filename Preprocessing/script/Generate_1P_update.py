...
def time_diff(sci_filename, aux_filename):
    ref_sci = re.compile(r'T2_POL_POLAR_SCI_(\d\d\d\d\d\d\d\d\d\d\d\d\d\d)_(\d\d\d\d\d\d\d\d\d\d\d\d\d\d)_1M.root')
...
# get filelist
ref_sci_1M = re.compile('T2_POL_POLAR_SCI_.*_1M\.root')
