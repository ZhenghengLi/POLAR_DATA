...
for x in file_list_sci_1p:
    ...
    while ppd_1m_end_time < sci_1p_end_time:
        ...
        if (ppd_1m_begin_time - pre_end_time).total_seconds() > 10:
            found_gap = True
            break
...
for k in sorted(sci_1p_ppd_1m_dict.keys()):
    ...
    outputfile = k.replace('.root', '_PPD_ENG_1M.root')
    print outputfile
    cur_ppd_file_list = [os.path.join(ppd_1m_dir, x) for x in sci_1p_ppd_1m_dict[k]]
    command = 'PPD_Split.py ' + ' '.join(cur_ppd_file_list) + ' -B ' + start_time + ' -E ' + stop_time + ' -o ' + os.path.join(output_dir, outputfile)
