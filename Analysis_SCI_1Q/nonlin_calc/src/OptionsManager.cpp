...
bool OptionsManager::parse(int argc_par, char** argv_par) {
    ...
    while (idx < argc_par - 1) {
        ...
        if (cur_par_str[0] == '-') {
            ...
            pol_event_filename_list.push_back(cur_par_str);
        }
    }
    if (pol_event_filename_list.empty()) return false;
    if (output_filename.IsNull()) {
        output_filename = "nonlin_output.root";
    }
    return true;
}
