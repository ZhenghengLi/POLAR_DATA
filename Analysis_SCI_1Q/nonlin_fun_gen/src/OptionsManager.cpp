...
bool OptionsManager::parse(int argc_par, char** argv_par) {
    ...
    while (idx < argc_par - 1) {
        ...
        if (cur_par_str[0] == '-') {
            ...
            case 'b':
                if (idx < argc_par - 1) {
                    bad_ch_filename = argv_par[++idx];
                    if (bad_ch_filename[0] == '-') {
                        return false;
                    }
                } else {
                    return false;
            ...
            if (input_filename.IsNull()) {
                input_filename = cur_par_str;
            } else {
                return false;
            }
        }
    }
    if (input_filename.IsNull()) return false;
    if (output_filename.IsNull()) {
        output_filename = "nonlin_fun.root";
    }
    return true;
}
...
void OptionsManager::print_help() {
    cout << "Usage:" << endl;
    cout << "  " << SW_NAME << "<SCI_1Q_1.root> <SCI_1Q_2.root> ... [-o <output.root>]" << endl;
    cout << endl;
    cout << "Options:" << endl;
    cout << "  -o <output.root>                 file that stores the nonlinearity result" << endl;
    cout << "  -b <bad_ch.txt>                  text file that stores the IDs of bad channels" << endl;
    cout << endl;
