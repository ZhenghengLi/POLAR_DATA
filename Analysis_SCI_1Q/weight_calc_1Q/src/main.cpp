...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 25; i++) {
        for (int j = 0; j < 64; j++) {
            if (j == 47) continue;
            if (i == 7 && j == 15) continue;
            if (i == 21 && j == 15) continue;
    ...
    t_pol_weight_tree->Branch("weight_is_bad",    &t_pol_weight.weight_is_bad,    "weight_is_bad/O"        );
    ...
    for (Long64_t q = begin_entry; q < end_entry; q++) {
        ...
        t_pol_weight.event_weight = -1;
        for (int i = 0; i < 25; i++) {
            t_pol_weight.mod_weight[i] = -1;
            for (int j = 0; j < 64; j++) {
                t_pol_weight.ch_weight[i][j] = -1;
            }
        }
        t_pol_weight.weight_is_bad = false;
        ...
        for (int i = 0; i < 25; i++) {
            ...
            // kill bad channels
            for (int j = 0; j < 64; j++) {
                if (t_pol_event.trigger_bit[i][j] && j == 47) {
                    t_pol_weight.weight_is_bad = true;
                    break;
                }
                if (t_pol_event.trigger_bit[i][j] && i == 7 && j == 15) {
                    t_pol_weight.weight_is_bad = true;
                    break;
                }
                if (t_pol_event.trigger_bit[i][j] && i == 21 && j == 15) {
                    t_pol_weight.weight_is_bad = true;
                    break;
                }
            }
