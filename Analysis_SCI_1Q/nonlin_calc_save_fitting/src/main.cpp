...
int main(int argc, char** argv) {
    ...
    TH1F* tri_hist[25][64][nbins_x];
    TH1F* all_hist[25][64][nbins_x];
    TH1F* eff_hist[25][64][nbins_x];
    ...
    for (int i = 0; i < 25; i++) {
        ...
        for (int j = 0; j < 64; j++) {
            ...
            for (int k = 1; k <= nbins_x; k++) {
                ...
                if (tri_stat < 30) {
                    tri_hist[i][j][k - 1] = NULL;
                    all_hist[i][j][k - 1] = NULL;
                    eff_hist[i][j][k - 1] = NULL;
                    continue;
                }
                tri_hist[i][j][k - 1] = static_cast<TH1F*>(h_spec_tri->Clone(Form("tri_hist_%02d_%02d_%02d", i + 1, j, k)));
                tri_hist[i][j][k - 1]->SetTitle(Form("tri_hist_%02d_%02d_%02d", i + 1, j, k));
                all_hist[i][j][k - 1] = static_cast<TH1F*>(h_spec_all->Clone(Form("all_hist_%02d_%02d_%02d", i + 1, j, k)));
                all_hist[i][j][k - 1]->SetTitle(Form("all_hist_%02d_%02d_%02d", i + 1, j, k));
                ...
                f_ratio->SetParameters(adc_left - 20, 15);
                h_spec_tri->Fit(f_ratio, "RQ");
                eff_hist[i][j][k - 1] = static_cast<TH1F*>(h_spec_tri->Clone(Form("eff_hist_%02d_%02d_%02d", i + 1, j, k)));
                eff_hist[i][j][k - 1]->SetTitle(Form("eff_hist_%02d_%02d_%02d", i + 1, j, k));
    ...
    cout << "writing eff fitting ..." << endl;
    output_file->cd();
    output_file->mkdir("eff_fitting");
    for (int i = 0; i < 25; i++) {
        output_file->mkdir(Form("eff_fitting/CT_%02d", i + 1));
        for (int j = 0; j < 64; j++) {
            output_file->mkdir(Form("eff_fitting/CT_%02d/channel_%02d", i + 1, j));
            output_file->cd(   Form("eff_fitting/CT_%02d/channel_%02d", i + 1, j));
            for (int k = 0; k < nbins_x; k++) {
                if (tri_hist[i][j][k] == NULL || all_hist[i][j][k] == NULL || eff_hist[i][j][k] == NULL) continue;
                tri_hist[i][j][k]->Write();
                all_hist[i][j][k]->Write();
                eff_hist[i][j][k]->Write();
            }
        }
    }
