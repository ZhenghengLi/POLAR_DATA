...
bool EventCanvas::open(const char* filename, int start, int step) {
    ...
    t_pol_event_tree_ = static_cast<TTree*>(t_pol_event_file_->Get("t_event"));
    if (t_pol_event_tree_ == NULL) {
        cout << "cannot find TTree t_event." << endl;
        return false;
    }

    t_pol_event_tree_->SetBranchAddress("time_aligned", t_pol_event_.time_aligned );
    t_pol_event_tree_->SetBranchAddress("lost_count",   &t_pol_event_.lost_count );
    t_pol_event_tree_->SetBranchAddress("trigger_bit", t_pol_event_.trigger_bit );
    t_pol_event_tree_->SetBranchAddress("multiplicity", t_pol_event_.multiplicity);
    t_pol_event_tree_->SetBranchAddress("energy_value", t_pol_event_.energy_value);
...
void EventCanvas::draw_event() {
    ...
    if (queue_flag_) {
        ...
        do {
            ...
            t_pol_event_tree_->GetEntry(entry_current_);
        } while (t_pol_event_.lost_count > 0);
    }
    ...
    cout << "TOTAL: " << trig_tot_mean - nontrig_tot_mean << "   " << endl;
