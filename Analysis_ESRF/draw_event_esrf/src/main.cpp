...
int main(int argc, char** argv) {
    ...
    TTree* t_rec_event_tree = static_cast<TTree*>(t_event_file->Get("t_beam_event"));
    if (t_rec_event_tree == NULL) {
        cout << "Cannot find TTree: t_beam_event." << endl;
        return 1;
    }
    ...
    t_rec_event_tree->SetBranchAddress("energy_adc", t_rec_event.energy_adc);
    ...
    for (int i = 0; i < 25; i++) {
        ...
        for (int j = 0; j < 64; j++) {
            ...
            energy_map->SetBinContent(ijtox(i, j) + 1, ijtoy(i, j) + 1,
                    (t_rec_event.energy_adc[k] > 0 ? t_rec_event.energy_adc[k] : 0 ));
        }
    }
